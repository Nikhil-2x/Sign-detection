...
while cap.isOpened(): 
    ...
    for bclass, bprob, bbox in zip(classes, probas, bboxes): 
        ...
        text_size, _ = cv2.getTextSize(frame_text, cv2.FONT_HERSHEY_SIMPLEX, 0.7, 2)
        text_w, text_h = text_size
        frame = cv2.rectangle(frame, (int(x1), int(y1)-text_h-5), (int(x1)+text_w, int(y1)), COLORS[bclass_idx], -1)

        # frame = cv2.putText(frame, frame_text, (int(x1),int(y1)), cv2.FONT_HERSHEY_DUPLEX, 2, (255,255,255), 4, cv2.LINE_AA)
        frame = cv2.putText(frame, frame_text, (int(x1), int(y1)-5), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2, cv2.LINE_AA)
